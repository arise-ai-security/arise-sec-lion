...
|---|---|---|---|---|
| Builder | Ensure the vulnerable base commit builds with sanitizer instrumentation. Edit `/src/build.sh` only when needed. | Run `cd <work_dir> && secb build`. `secb build` replays `repo_changes.diff` if present and delegates the actual build to `/usr/local/bin/compile`, which runs `$SRC/build.sh`. | Arise Builder contract: `/testcase/base_commit_hash`, `/src/build.sh`, `/testcase/repo_changes.diff`, `/testcase/binary_paths.txt` (**not** `packages.txt`) | SEC-bench helper: `../SEC-bench/secb/preprocessor/templates/secb_helper.sh.j2:4-35`; Arise contract: `plugins/security/deliverables.py:65-74` |
| Exploiter | Use existing PoC artifacts first and ensure `secb repro` triggers the expected sanitizer error. | Run `secb repro` against the Builder-declared binary. In Arise patch images the shipped PoC inputs are baked in (arbitrary filenames), but Arise re-seeds `repro.sh` to an exit-2 skeleton, so the agent authors the reproducer. | Arise Exploiter contract: `/testcase/poc_path.txt` (explicit pointer, no `poc*` glob), `/testcase/repro.sh` | SecVerifier prompt: `../SecVerifier/prompts/instructions/exploiter_agent_instruction.j2:13-25`; Arise contract: `plugins/security/deliverables.py:30,73`; `prompts/domains/secbench/phases/exploit.j2:8` |
...
> Cross-ref: for the as-built role/artifact contract these facts feed, see
> [`SYSTEM_REFERENCE.md`](../SYSTEM_REFERENCE.md) Part I (SEC-bench substrate) and
> §III.4 (artifact contract).

| Fact | Explanation | Evidence |
|---|---|---|
| `secb build` IS the Builder's primary build/validation command. | The prompt instructs "Run `secb build` from the CVE work directory" and states "`secb build` is the primary build/validation command." Direct execution of `build.sh` is troubleshooting only. | `prompts/domains/secbench/phases/build.j2:7,15,17` |
| `build.sh` is the editable recipe `secb build` runs, not a substitute command. | The prompt keeps `build.sh` as "the editable project build recipe, but direct execution of it is only troubleshooting"; agents repair it only when `secb build` fails. | `prompts/domains/secbench/phases/build.j2:8,16` |
| `packages.txt` is NOT a Builder deliverable. | It has no key in `deliverables.py` and is deliberately excluded from the mandatory contract; drift tests assert it never appears as a mandated path or in the rendered prompt. | `plugins/security/deliverables.py` (no `packages` key); `plugins/security/tests/test_roles.py:131`; `plugins/security/tests/test_prompt_building.py:722` |
| The current Exploiter prompt uses `repro.sh` as the deliverable. | This creates a separate reproducer contract from SEC-bench's baked `secb repro` command; `secb repro` is still run for validation. | `prompts/domains/secbench/phases/exploit.j2:12,22` |
| The contract uses an explicit `poc_path.txt` pointer, NOT a `/testcase/poc*` glob. | Repro-Creator writes `poc_path.txt` with the exact selected PoC path; the prompt explicitly says "do not assume any filename prefix." | `plugins/security/deliverables.py:30`; `prompts/domains/secbench/phases/exploit.j2:8,9,23` |
| The current Exploiter prompt still prioritizes provided PoC artifacts. | It says to select the shipped PoC under `/testcase` first and only create a new PoC when none matches the CVE. | `prompts/domains/secbench/phases/exploit.j2:7,10` |
