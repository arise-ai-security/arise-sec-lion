...
from core.domain.events.events import (
    ...
    WorkerCostRecorded, OperationFinished, RunStarted,
...
async def main():
    ...
    def subtask(desc, si=0, deps=None):
        return Subtask(description=desc, config={}, depends_on=deps or [], sibling_index=si)

    # ----- BOSS -----
    add(boss_id, AgentCreated, 0, role="boss", config={}, sibling_index=0, depends_on=[])
    ...
    add(boss_id, ChildSpawned, 7, child_id=mgr1_id, child_role="pending", subtask=subtask("Identify vulnerable deserialization entry points", 0), child_config={})
    add(boss_id, ChildSpawned, 7.5, child_id=mgr2_id, child_role="pending", subtask=subtask("Generate and apply security patch", 1, [0]), child_config={})
    add(boss_id, ChildSpawned, 8, child_id=mgr3_id, child_role="pending", subtask=subtask("Verify patch correctness and run regression tests", 2, [1]), child_config={})

    # ----- MANAGER 1: Analysis -----
    add(mgr1_id, AgentCreated, 8.5, role="pending", parent_id=boss_id, config={}, sibling_index=0, depends_on=[])
    ...
    add(mgr1_id, ChildSpawned, 14.5, child_id=w1_id, child_role="pending", subtask=subtask("Scan source for ObjectInputStream.readObject() calls", 0), child_config={})
    add(mgr1_id, ChildSpawned, 15, child_id=w2_id, child_role="pending", subtask=subtask("Analyze transformer chain gadget classes", 1, [0]), child_config={})

    # ----- WORKER 1 -----
    add(w1_id, AgentCreated, 15.5, role="pending", parent_id=mgr1_id, config={}, sibling_index=0, depends_on=[])
    ...
    # ----- WORKER 2 -----
    add(w2_id, AgentCreated, 15.5, role="pending", parent_id=mgr1_id, config={}, sibling_index=1, depends_on=[0])
    ...
    add(mgr2_id, AgentCreated, 46.5, role="pending", parent_id=boss_id, config={}, sibling_index=1, depends_on=[0])
    ...
    add(mgr2_id, ChildSpawned, 52.5, child_id=w3_id, child_role="pending", subtask=subtask("Add deserialization filter", 0), child_config={})
    add(mgr2_id, ChildSpawned, 53, child_id=w4_id, child_role="pending", subtask=subtask("Compile and verify patch", 1, [0]), child_config={})

    # ----- WORKER 3 (patch writer — VerificationFailed + RetryScheduled) -----
    add(w3_id, AgentCreated, 53.5, role="pending", parent_id=mgr2_id, config={}, sibling_index=0, depends_on=[])
    ...
    add(w4_id, AgentCreated, 53.5, role="pending", parent_id=mgr2_id, config={}, sibling_index=1, depends_on=[0])
    ...
    add(mgr3_id, AgentCreated, 100.5, role="pending", parent_id=boss_id, config={}, sibling_index=2, depends_on=[1])
    ...
    add(mgr3_id, ChildSpawned, 106.5, child_id=w5_id, child_role="pending", subtask=subtask("Run exploit PoC against patched code", 0), child_config={})
    add(mgr3_id, ChildSpawned, 107, child_id=w6_id, child_role="pending", subtask=subtask("Run full regression test suite", 1), child_config={})

    # ----- WORKER 5 (exploit PoC test) -----
    add(w5_id, AgentCreated, 107.5, role="pending", parent_id=mgr3_id, config={}, sibling_index=0, depends_on=[])
    ...
    add(w6_id, AgentCreated, 107.5, role="pending", parent_id=mgr3_id, config={}, sibling_index=1, depends_on=[])
