...
async def db_stale_analyzing_children(
    ...
    q = (
        ...
        "    'AgentCreated','TaskAssigned','OperationStarted','ComplexityEvaluated','OperationFinished',"
        "    'StatusChanged','WorkCompleted','WorkFailed','RetryScheduled','AgentExecutionStarted'"
        "  )"
        "), latest_state AS ("
        ...
        "      WHEN event_type='AgentExecutionStarted' THEN 'analyzing' "
        "      WHEN event_type='AgentCreated' THEN 'analyzing' "
        "      WHEN event_type='TaskAssigned' THEN 'analyzing' "
        "      WHEN event_type='OperationStarted' THEN 'analyzing' "
        "      WHEN event_type='ComplexityEvaluated' THEN 'analyzing' "
        "      WHEN event_type='OperationFinished' THEN 'analyzing' "
        "      ELSE 'unknown' "
        "    END AS status "
        "  FROM latest_state"
        "), last_event AS ("
