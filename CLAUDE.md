...
- **Deterministic procedure tier (flag-gated)** -- when `settings.orchestration.procedural_dispatch` is on, `Build-Verifier`, `Exploit-Validator`, `Patch-Applier`, and `Patch-Validator` run under Host control with zero LLM turns via `ProcedureExecutorPort` (`core/ports/procedure_ports.py`), emitting Host-computed `ProcedureExecutionStarted/Finished` evidence; a failed procedure escalates to exactly one agentic retry. Commands still execute in the run's mutable shared container, so the fresh external evaluator remains confirmatory authority. Default off binds `NullProcedureExecutor`. Security executor and mechanical criteria: `.claude/docs/plugins-security.md`.
