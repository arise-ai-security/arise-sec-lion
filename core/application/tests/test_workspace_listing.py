...
Legacy mode lists everything except src/ — an out-of-tree
...
def _populate_workspace(tmp_path: Path) -> None:
    ...
    (tmp_path / "artifacts" / "run.sh").write_text("#!/bin/sh\n")
    (tmp_path / "artifacts" / "result.log").write_text("result\n")
    ...
    (tmp_path / "container-exec").write_text("#!/bin/sh\n")


def test_legacy_listing_skips_only_src(tmp_path: Path) -> None:
    # Given: a workspace with artifacts, a build tree, sources, and a helper
    ...
    assert "artifacts/run.sh" in listing
    assert "work/demo-build/CMakeFiles/demo.o" in listing
    assert "container-exec" in listing
...
def test_whitelist_lists_only_included_dirs(tmp_path: Path) -> None:
    # Given
    _populate_workspace(tmp_path)
    service = _service(tmp_path, workspace_listing_dirs=("artifacts",))
    ...
    assert listing is not None
    assert "artifacts/run.sh" in listing
    assert "artifacts/result.log" in listing
    assert "demo.o" not in listing
    assert "container-exec" not in listing
    assert "src/main.c" not in listing


def test_entry_cap_summarizes_overflow(tmp_path: Path) -> None:
    # Given: three artifact files and a cap of one entry
    _populate_workspace(tmp_path)
    (tmp_path / "artifacts" / "details.txt").write_text("details\n")
    service = _service(
        tmp_path,
        workspace_listing_dirs=("artifacts",),
