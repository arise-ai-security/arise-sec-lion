...
|---|---|---|
| `[Build-Verifier]` | `secb_build_validation` | `secb build`, validate the exact Builder repository baseline, then validate every declared executable ELF and initialize its configured sanitizer runtime |
...
- **Frozen handoff.** A sealed replay contract binds the CVE/base/sanitizer/work directory, dataset exit oracle, source HEAD and Builder-baseline digest, reproducer, PoC/pointer bytes, every declared binary's path/mode/digest, resolved replay argv/stdin mode, and any required project-local library's SONAME, candidate, resolved target, target digest, and effective loader path. Exploit success revalidates and recaptures that identity after all three direct runs before the Host freezes it with the verdict hash. Patch application additionally seals the plan, rendered diff, and replay identity. Patch validation rejects any source, build-input, target-binary, replay-input, library-identity, plan, or patch drift before applying, rebuilding, and replaying the frozen target.
- **Controlled execution.** Procedure commands use argv vectors, never `bash -lc`. Docker preserves the image's OSS-Fuzz variables while the Host fixes `PATH`, `LC_ALL=C`, `BASH_ENV=/dev/null`, and `ENV=/dev/null`; sanitizer-specific halt options are supplied for direct replay. The Host-owned deadlines guard external-process liveness, not predicate evaluation. When a direct ELF reports a canonical missing shared-library SONAME, the Host may resolve one unique safe candidate below the CVE work directory and freeze its concrete directory into the replay environment; missing, ambiguous, or path-escaping candidates fail closed. Declared-binary startup probes stream output. On the exact configured sanitizer marker, the Host stops the probe and resets the shared container immediately, returning `STOPPED_AFTER_MARKER`, not `TIMED_OUT`; a deadline is `TIMED_OUT` and fails when the marker was not observed. Every startup-probe outcome resets the container so detached descendants cannot survive. Every ordinary build, patch, exploit-replay, and post-patch-replay timeout remains fatal. Failure to reset the shared container is an infrastructure error.
...
| Gate | PASS | FAIL |
|---|---|---|
| Build | `secb build` exits 0; the byte-exact `repo_changes.diff` matches repository state under `CVEInstance.work_dir`; every declared absolute path is a non-empty executable ELF whose streamed startup probe reaches the exact ASan/MSan/UBSan marker and returns `STOPPED_AFTER_MARKER` after the Host stops and resets it | build timeout/nonzero exit, baseline mismatch, missing/relative path, non-ELF/non-executable target, startup deadline without the marker (`TIMED_OUT`), missing/ambiguous/unsafe project library, or missing/unsupported sanitizer runtime |
| Exploit preflight | canonical structured direct-argv replay; PoC file and non-empty binaries resolve; source/Builder baseline, pointer bytes, every binary path/mode/digest, and any required project-library identity freeze; configured sanitizer initializes (a declared zero-byte PoC is valid) | unsafe replay grammar, missing/unresolved or symlinked artifact, source/build/library drift, empty/changed binary, ambiguous/unsafe library resolution, or sanitizer failure |
