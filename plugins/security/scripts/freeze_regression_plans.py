"""Generate and freeze arm-independent container regression plans.

For every ``(instance_id, base_commit)`` in the confirmatory cohort:
...
Usage::
    ...
    uv run python -m plugins.security.scripts.freeze_regression_plans \\
        --study experiments/b4-confirmatory-cohort \\
    ...
    *,
    ...
    document = {
        ...
        "meta": {
            "generator_model": _GENERATOR_MODEL,
            "dataset_revision": dataset_revision,
            "arm_identity": "identical_plan_for_N1_and_B4",
    ...
    if failures:
        ...
        for item in failures:
            logger.error("- %s", item)
        # Do not write a partial freeze — confirmatory requires the full cohort.
        logger.error(
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--study",
        type=Path,
        default=Path("experiments/b4-confirmatory-cohort"),
