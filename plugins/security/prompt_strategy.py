...
def role_from_task(task_description: str) -> Role | None:
    ...
    labels resolve to None. Worker rendering distinguishes those two cases and
...
class SecBenchPromptStrategy:
    ...
    def extend_worker_prompt(
        ...
        # Role-ownership gate: a leaf stamped with a catalog role receives one
        # explicitly allowlisted template. Never derive a template path from the
        # task text. Canonical phase labels retain the whole-phase/fused path.
        # Reporter is both the phase and its only catalog role. The role-fused
        # treatment needs the phase contract; B4 uses the catalog-role template.
        if self._role_fused and label is not None and label.lower() in _PHASE_TO_BRANCH:
            worker_role = None
