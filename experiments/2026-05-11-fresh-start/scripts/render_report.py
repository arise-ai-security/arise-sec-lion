...
def _render(
    ...
    groups = _load_groups()
    cells_map = manifest.get("cells") or {}
    # BUG-CR1: keep the rendered cell list in sync with collect.py's filter
    # so smoke cells registered in `cells:` do not appear in headline reports.
    headline = manifest.get("headline_cells")
    if isinstance(headline, list) and headline:
        allowlist = {str(name) for name in headline}
        cells_map = {name: entry for name, entry in cells_map.items() if name in allowlist}
    cells = [
        {
            "id": name,
        ...
        for name, entry in cells_map.items()
    ]
    table_rows = [
        {
            "cell": row["cell"],
