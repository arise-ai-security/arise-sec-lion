...
def _prepare_run_manifest_payload(
    ...
    *,
    ...
    Does not write anything. Raises FileNotFoundError if the run manifest
    is absent so callers can fail fast. ``projection_status`` (audit N-2)
    carries forward whether `events.jsonl` was generated successfully
    (``"ok"``) or the projection raised (``"failed"``); `collect.py`
    excludes failed rows so a missing/empty events.jsonl no longer
    masquerades as a clean zero.
    """
...
def register_run(
    *,
    ...
    ``experiments.shared.scripts.collect``. ``projection_status`` defaults
    to ``"ok"``; the harness passes ``"failed"`` when event projection
    raised so downstream rollups can exclude the row (audit N-2).
    """
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Sample index within (cell, task)",
    )
    parser.add_argument(
        "--output-directory",
